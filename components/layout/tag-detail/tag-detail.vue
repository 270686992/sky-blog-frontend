<template>
  <div class="tags-detail-container">
    ...
    <article class="article-list">
      <article-list :articles="articles" :loading="loading" :total="total" :categories="categories" @loadMore="onLoadMore"></article-list>
    </article>
  </div>
</template>
...
export default {
  ...
  methods: {
    onLoadMore() {
      ...
      if (this.type === 'tag') {
        this.$store.dispatch('tag/getMoreArticlesByTag', params)
      } else  {
        this.$store.dispatch('tag/getMoreArticlesByCategory', params)
      }
    },
  }
}
