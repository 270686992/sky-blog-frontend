<template>
  <div class="editor-container">
    <img class="avatar" :src="avatar" :alt="customer.nickname || '匿名用户'">
      ...
      <transition-group class="slide-wrapper" tag="div" name="list-slide">
        ...
        <section class="list-slide-item editor-item" :key="2">
          <div
            ...
            :placeholder="isMessageEditor ? '留言...' : '评论...'"
          ></div>
        </section>
...
export default {
  ...
  computed: {
    avatar() {
      return gravatar(this.customer.email)
    },
  ...
  methods: {
    ...
    getInValidMsg() {
      ...
      if (!this.form.content  || !this.form.content.replace(/\s/g, '')) {
        if (this.form.content.length > 500 || this.form.content.split('\n').length > 30) {
          return `${this.isMessageEditor ? '留言' : '评论'}内容不能超过500个字且控制在30行以内`
        }
